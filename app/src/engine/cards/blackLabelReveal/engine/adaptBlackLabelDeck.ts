 ...
 * the real `item.traits` field by the fan-reveal's own inspect sheet
 * (BlackLabelFanReveal.tsx's DescriptionPanel), so duplicating them onto VaultCardData here would
...
export function adaptPulledItemsToBlackLabelDeck(items: PulledOwnedItem[], sku: PackSku): VaultCardData[] {
  return adaptPulledItemsToVaultDeck(items, sku, "BL").map((card) => ({
    ...card,
    stock: OBSIDIAN,
    accent: "#d8b877",
